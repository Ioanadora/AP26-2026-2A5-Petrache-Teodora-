...
public class Robot extends Thread {
    ...
    public void run() {
        ...
        while (!game.isGameOver()) {
            // aici
            Position target = game.getNearestSightedBunny(position);
            if (target != null) {
                if (position.equals(target)) {
                    game.clearSighting(target);
                }
                moveTowards(target);
            } else {
                moveSystematic();
    ...
    private void moveTo(Position next) {
        ...
        // aicic
        List<Position> currentBunnies = game.getAllBunnyPositions();
        for (Position bunnyPos : currentBunnies) {
            int distance = Math.abs(position.row - bunnyPos.row) + Math.abs(position.col - bunnyPos.col);

            // aici
            if (distance <= 2) {
                for (Bunny bunny : game.getBunnies()) {
                    if (bunny.getPosition().equals(bunnyPos)) {
                        game.shareSighting(bunny, bunnyPos);
                        break;
                    }
                }
            }
            ...
            if (position.equals(bunnyPos)) {
                for (Bunny bunny : new ArrayList<>(game.getBunnies())) {
                    if (bunny.getPosition().equals(position)) {
                        System.out.println("[ROBOT]Caught bunny at " + position);
                        bunny.catchBunny();
                        game.removeBunny(bunny);
                        break;
                    }
                }
            }
